...
function BestCost = RLDVAO(CostFunction, dim, VarSize, VarMin, VarMax, MaxIt, nPop, omega, psi, lambda, mu, mu_damp, delta)
        ...
        for i = 1:nPop
            ...
            for j = 1:nPop
                if pop(j).Cost < pop(i).Cost
                    ...
                    mu_max =0.2;
                    mu_min =0.001;
                    mu = mu_max - (mu_max - mu_min) * it / MaxIt;% 自适应变异策略 
...
%         mu = mu * mu_damp;
        ...
        disp(['In Iteration No ' num2str(it) ': RLDVAO Best Cost Is = ' num2str(BestCost(it))]);
    end
