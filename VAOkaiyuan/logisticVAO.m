%% logisticVAO Algorithm Function

function [BestCost,mu_history5] = logisticVAO(CostFunction, dim, VarSize, VarMin, VarMax, MaxIt, nPop, omega, psi, lambda, mu, mu_damp, delta)
...
%                      % 自适应变异策略
                    ...
                    x=unifrnd(-1, 1, VarSize);
                    newsol.Position = pop(i).Position ...
                        + beta *(1 ./ (1 + exp(-x))).* (pop(j).Position - pop(i).Position) ...
                        + mu * e;
...
% Merge 合并
        ...
        disp(['In Iteration No ' num2str(it) ': logisticVAO Best Cost Is = ' num2str(BestCost(it))]);
    end
