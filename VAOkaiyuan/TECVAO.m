function BestCost = TECVAO(CostFunction, dim, VarSize, VarMin, VarMax, MaxIt, nPop, omega, psi, lambda, mu, mu_damp, delta)
    ...
    for it = 1:MaxIt
        ...
        for i = 1:nPop
            ...
            for j = 1:nPop
               ...
               mu=0.45*1.2*(exp(-((it/MaxIt)*pi))*abs(cos(5*8*(it/MaxIt)*pi))); %自适应变异策略 
...
%         mu = mu * mu_damp;
        ...
        disp(['In Iteration No ' num2str(it) ': TECVAO Best Cost Is = ' num2str(BestCost(it))]);
    end
